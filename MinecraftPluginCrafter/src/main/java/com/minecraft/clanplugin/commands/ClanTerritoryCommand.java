...
public class ClanTerritoryCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (subCommand) {
            ...
            case "zoom":
                return handleZoom(player);
            case "clanmates":
                return handleClanmates(player);
            case "info":
    ...
    /**
     * Handles the zoom command for the dynamic map.
     * 
     * @param player The player
     * @return True if the command was handled
     */
    private boolean handleZoom(Player player) {
        // Check if player has permission
        if (!player.hasPermission("clan.territory.zoom")) {
            player.sendMessage(ChatColor.RED + "You don't have permission to use the map zoom feature!");
            return true;
        }
        
        // Check if player has a map active
        if (!plugin.getTerritoryMap().isUsingMap(player.getUniqueId())) {
            player.sendMessage(ChatColor.RED + "You don't have a dynamic map active! Use /clan territory dynamicmap first.");
            return true;
        }
        
        // Cycle zoom level
        String newZoom = plugin.getTerritoryMap().cycleZoomLevel(player);
        player.sendMessage(ChatColor.GREEN + "Map zoom level changed to " + ChatColor.YELLOW + newZoom);
        return true;
    }
    
    /**
     * Handles the clanmates command to show clan members on the map.
     * 
     * @param player The player
     * @return True if the command was handled
     */
    private boolean handleClanmates(Player player) {
        ...
        if (clan == null) {
            player.sendMessage(ChatColor.RED + "Error accessing your clan data.");
            return true;
        }
        
        // Show clan members on map
        plugin.getTerritoryMap().showClanMembersOnMap(player, clan);
        return true;
    }
    ...
    private void sendHelpMessage(Player player) {
        ...
        player.sendMessage(ChatColor.YELLOW + "/clan territory dynamicmap" + ChatColor.WHITE + " - Gives you a real-time territory map item");
        player.sendMessage(ChatColor.YELLOW + "/clan territory zoom" + ChatColor.WHITE + " - Changes the zoom level of your dynamic map");
        player.sendMessage(ChatColor.YELLOW + "/clan territory clanmates" + ChatColor.WHITE + " - Shows clan members on your map");

...
public class ClanCommand implements CommandExecutor {
    ...
    private boolean handleGUI(Player player) {
        ...
        clanInfo.add("Level: " + ChatColor.GOLD + playerClan.getLevel());
        clanInfo.add("XP: " + ChatColor.YELLOW + playerClan.getExperience() + " points");
        
        // Add next level info if not at max level
        int nextLevelXP = plugin.getProgressionManager().getExperienceForNextLevel(playerClan);
        if (nextLevelXP > 0) {
            clanInfo.add("Next Level: " + ChatColor.YELLOW + nextLevelXP + " XP needed");
        } else {
            clanInfo.add(ChatColor.GREEN + "Maximum level reached!");
        }
        ...
        // Create clan progression item
        List<String> progressionLore = new ArrayList<>();
        progressionLore.add("");
        progressionLore.add(ChatColor.GRAY + "View and manage your clan's");
        progressionLore.add(ChatColor.GRAY + "progression and level benefits.");
        progressionLore.add("");
        progressionLore.add(ChatColor.YELLOW + "Current Level: " + ChatColor.GOLD + playerClan.getLevel());
        progressionLore.add(ChatColor.YELLOW + "Current XP: " + ChatColor.WHITE + playerClan.getExperience());
        
        // Add next level info if not at max level
        int expForNextLevel = plugin.getProgressionManager().getExperienceForNextLevel(playerClan);
        if (expForNextLevel > 0) {
            progressionLore.add(ChatColor.YELLOW + "XP Needed: " + ChatColor.WHITE + expForNextLevel);
        } else {
            progressionLore.add(ChatColor.GREEN + "Maximum level reached!");
        }
        
        progressionLore.add("");
        progressionLore.add(ChatColor.AQUA + "Click to view level benefits");
        ...
        gui.setItem(33, progressionItem);
        
        // Create clan skills item
        List<String> skillsLore = new ArrayList<>();
        skillsLore.add("");
        skillsLore.add(ChatColor.GRAY + "View and manage your clan's");
        skillsLore.add(ChatColor.GRAY + "skill trees and member specializations.");
        skillsLore.add("");
        skillsLore.add(ChatColor.DARK_AQUA + "Role-based specializations:");
        skillsLore.add(ChatColor.GRAY + "• " + ChatColor.GRAY + "Miner: " + ChatColor.WHITE + "Mining bonuses");
        skillsLore.add(ChatColor.GRAY + "• " + ChatColor.DARK_GREEN + "Farmer: " + ChatColor.WHITE + "Farming yields");
        skillsLore.add(ChatColor.GRAY + "• " + ChatColor.YELLOW + "Builder: " + ChatColor.WHITE + "Construction speed");
        skillsLore.add(ChatColor.GRAY + "• " + ChatColor.DARK_RED + "Hunter: " + ChatColor.WHITE + "Combat strength");

...
public class ClanEconomyCommand implements CommandExecutor {
    ...
    private boolean handleDeposit(Player player, String[] args) {
        ...
        // This would normally check if the player has enough money in their personal account
        // Since we're just designing the plugin, we'll assume they do
        
        if (economy.depositToClan(clanName, amount)) {
    ...
    private boolean handleWithdraw(Player player, String[] args) {
        ...
        if (economy.withdrawFromClan(clanName, amount)) {
            player.sendMessage(ChatColor.GREEN + "Successfully withdrew " + 
                              economy.formatCurrency(amount) + ChatColor.GREEN + " from clan bank!");
            ...
            // This would normally add money to the player's personal account
            // Since we're just designing the plugin, we'll assume it does
        } else {
    ...
    private boolean handleCollect(Player player) {
        ...
        double collected = economy.collectTaxes(clanName, player.getUniqueId());
